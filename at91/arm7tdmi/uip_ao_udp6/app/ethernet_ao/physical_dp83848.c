 ...
 */
void phy_d83848_init( const unsigned char * macaddr, void (*handler)(void)) {
...
void  d83848_init(const unsigned char * mac, void vhandler(void) );
/*---------------------------------------------------------------------------*/
void phy_dev_init() {
	...
	// add a f*cking typecast since every shitty application has to re-create every typedef again and again
	 phy_d83848_init(  &MacAddress[0], ISR_Emac);
}
