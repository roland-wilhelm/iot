    ...
    switch rec    % discriminate based on the record number
        ...
        case 14   %  QS_QF_ACTIVE_POST_FIFO
            ...
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
               [tmp(1) tmp(3) (tmp(6)+1) (tmp(7)+1) tmp(2) tmp(4) tmp(5) 0 1];

        case 15   %  QS_QF_ACTIVE_POST_LIFO
            tmp = sscanf(line, '%*u %u %u %u %u %u %u %u')';
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
               [tmp(1) tmp(3) (tmp(6)+1) (tmp(7)+1) tmp(2) tmp(4) tmp(5) 1 1];

        case 16   %  QS_QF_ACTIVE_GET
            tmp = sscanf(line, '%*u %u %u %u %u %u %u')';
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
                [tmp(1) tmp(3) (tmp(6)+1) NaN tmp(2) tmp(4) tmp(5) 0 -1];

        case 17   %  QS_QF_ACTIVE_GET_LAST
            tmp = sscanf(line, '%*u %u %u %u %u %u')';
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
                [tmp(1) tmp(3) NaN NaN tmp(2) tmp(4) tmp(5) 0 -1];

        case 18   %  QS_QF_EQUEUE_INIT
            tmp = sscanf(line, '%*u %u %u')';
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
                [NaN tmp(1) NaN (tmp(2)+1) NaN NaN NaN 0 0];
        ...
        case 21   %  QS_QF_EQUEUE_GET
            tmp = sscanf(line, '%*u %u %u %u %u %u %u')';
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
                [tmp(1) tmp(3) (tmp(6)+1) 0 tmp(2) tmp(4) tmp(5) 0 -1];

        case 22   %  QS_QF_EQUEUE_GET_LAST
            tmp = sscanf(line, '%*u %u %u %u %u %u')';
            Q_EQUEUE(size(Q_EQUEUE,1)+1,:) = ...
                [tmp(1) tmp(3) 0 0 tmp(2) tmp(4) tmp(5) 0 -1];
        ...
        case 26   %  QS_QF_PUBLISH_ATTEMPT
            Q_PUB(size(Q_PUB,1)+1,:) = [sscanf(line, '%*u %u %u %u %u')' 0];

        case 27   %  QS_QF_PUBLISH
            Q_PUB(size(Q_PUB,1)+1,:) = sscanf(line, '%*u %u %u %u %u %u')';
        ...
        case 38   %  QS_QF_TIMEEVT_PUBLISH
            Q_TIME(size(Q_TIME,1)+1,:) = ...
                [sscanf(line, '%*u %u %u %u')' NaN NaN NaN 0];
